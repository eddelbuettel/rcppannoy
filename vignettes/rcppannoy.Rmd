...
The [Annoy](https://github.com/spotify/annoy) \citep{Github:annoy} C++ library implements a quick and simple method for approximate nearest neighbor (oh yeah) searching.
The _RcppAnnoy_ package \citep{CRAN:RcppAnnoy} provides a centralized resource for developers to use this code in their own R packages.
...
The Annoy library (or specifically, the `Kiss64Random` class) uses a fixed seed for its random number generation.
